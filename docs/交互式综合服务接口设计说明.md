...
-	zeroMQ 过滤器格式定义(暂定)
	...
	1.	整体格式采用`服务名.req/rep.指令.地址`的格式。服务名列表见下。
	2.	`req`表示向末端设备请求，`rep`表示末端数据的应答，二选一。
	3.	各个服务的指令定义见各自协议部分。
	4.	地址均为末端设备的物理地址，监控终端地址采用4位显示方式，左边不足的补0.
...
message UserLogin {
    ...
    repeated int32 area_r = 9;  // 可读区域
    repeated int32 area_w = 10;  // 可写区域
    repeated int32 area_x = 11;  // 可操作区域
}
```
...
message AreaInfo {
    message AreaView {
        ...
        repeated int32 tml_id = 3;  // 下属设备id
    }
    Head head = 1;
...
message rqTmlInfo {
    Head head = 1;
    repeated int32 data_mark = 5;  // 请求数种类,1-baseinfo,2-gisinfo,4-rtuinfo,6-sluinfo,7-lduinfo,8-alsinfo,9-mruinfo,10-esuinfo(好烦,不想做)
    int32 tml_id = 6;  // 请求的设备逻辑id, 0-返回所有数据
}
```
...
message TmlInfo {
    ...
    repeated int32 data_mark = 3 [packed=true];  // 请求数种类,1-baseinfo,2-gisinfo,4-rtuinfo,6-sluinfo,7-lduinfo,8-alsinfo,9-mruinfo,10-esuinfo
}
```
...
-	注意:所有控制类接口,提供tml_id和phy_id两个字段,任选其一即可,若同时填写,则以phy_id为准

##### **zeroMQ推送说明**

-	推送给客户端的终端应答消息体： 采用MsgWithCtl结构发送
...
message rqRtuCtl {
    message RtuDo {
        ...
        repeated int32 phy_id = 4 [packed=true];  // 要操作的设备物理地址列表,该列表中的终端均执行相同的操作(高优先级,仅管理员或预置用户有效)
...
message rqSluDataGet {
    Head head = 1;
    repeated int32 tml_id = 2 [packed=true];  // 要操作的设备逻辑地址列表,该列表中的终端均执行相同的操作
    repeated int32 phy_id = 7 [packed=true];  // 要操作的设备物理地址列表,该列表中的终端均执行相同的操作(高优先级,仅管理员或预置用户有效)
...
message rqSluitemDataGet {
    Head head = 1;
    repeated int32 tml_id = 2 [packed=true];  // 要操作的设备逻辑地址列表,该列表中的终端均执行相同的操作
    repeated int32 phy_id = 6 [packed=true];  // 要操作的设备物理地址列表,该列表中的终端均执行相同的操作(高优先级,仅管理员或预置用户有效)
...
message rqSluCtl {
    Head head = 1;
    repeated int32 tml_id = 2 [packed=true];  // 要操作的设备逻辑地址列表,该列表中的终端均执行相同的操作
    repeated int32 phy_id = 13 [packed=true];  // 要操作的设备物理地址列表,该列表中的终端均执行相同的操作(高优先级,仅管理员或预置用户有效)
    int32 cmd_idx = 3;  // 序号
...
message QueryDataSlu {
    message DataSluView {
        ...
        repeated int32 zigbee_channel = 9;  // zigbee信道(1-16)
    }
...
message rqQuerySmsRecord {
    Head head = 1;
    repeated int32 tels = 3 [packed=true];  // 查询号码,长度11,可填写多个,可选
...
> 接口名称: submitalarm (*post*\)  
> 接口将中间层提交的故障通过zmq-pub广播  
> zmq-pub 广播过滤器格式: `jkdb.rep.alarm.<报警-1/消警-0>.<故障id>.<设备id>`  
> zmq-pub 广播消息体格式: `AlarmView 结构体序列化后的二进制字符串`

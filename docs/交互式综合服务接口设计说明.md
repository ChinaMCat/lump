...
|2017-08-03|徐源|1. 整加光照度查询接口<br>2. 修复sql注入攻击漏洞|
...
```protobuf
message rqQueryEventsTimetableDo {
    Head head = 1;
...
```protobuf
message QueryEventsTimetableDo {
...
message rqQueryRtuTimeTableBind {
    Head head = 1;
    repeated int32 tml_id = 2 [packed=true];  // 设备逻辑地址
    int32 data_mark = 3;  // 查询标记，0-查询当天，1-查询指定日期(暂不支持)
...
- zmq-sub 订阅过滤器：`jkdb.rep.通讯层端口号.alarm.是否报警.故障id.设备id`
- zmq-sub 消息体格式：`MsgWithCtrl.SubmitAlarm()`
```
message SubmitAlarm {
    message AlarmView {
        optional int64 dt_create = 1;  // 故障产生时间
        optional int32 err_id = 2;  // 故障id
        optional int32 tml_id = 3;  // 设备id
        optional int32 loop_id = 4;  // 回路id,从1开始,非回路故障填0
        optional int32 lamp_id = 5;  // 灯头id,从1开始,非灯头故障填0
        optional int32 err_count = 6;  // 在一定时间内重复产生的故障数量
        optional int64 dt_remove = 8;  // 故障消除时间
        optional int32 is_alarm = 9;  // 1-报警,0-消警
        optional int32 alarm_src = 10;  // 报警来源
    }
    // Head head = 1;
...
- zmq-sub 订阅过滤器：`jkdb.rep.通讯层端口号.setting.设备id`
- zmq-sub 消息体格式：`MsgWithCtrl.SubmitSettingChange()`
```
message SubmitSettingChange {
    // 参数类型，1-参数变化（含增删改），2-时间表变化（含单灯，终端，增删改），3-定时任务变化（增删改）
    optional int32 setting_type = 1;

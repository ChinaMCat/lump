...
- ZMQ-PUB广播说明
	- 过滤器格式： `tcs.rep.通讯服务端口号.消息指令.物理地址`（消息指令详见MsgWithCtl结构体head.cmd说明）
...
message rqLduDataGet {
    Head head = 1;
    repeated int32 tml_id = 2 [packed=true];  // 要操作的设备逻辑地址列表,该列表中的终端均执行相同的操作
    repeated int32 loop_id = 3 [packed=true];  // 回路标示，必填，3090设备范围1-6，1090设备范围1-2
}
```
...
message rqQueryDataErr {
    ...
    int64 dt_start = 5;  // 起始年月日时间,转换为long传输(1970-1-1开始的秒数)
    int64 dt_end = 6;  // 结束年月日时间,转换为long传输(1970-1-1开始的秒数)
    int32 type = 7;  // 类型,0-现存,1-历史,2-现存仅返回数量（存放在head.paging_record_total），3-历史仅返回数量（存放在head.paging_record_total）
    repeated int32 tml_id = 9 [packed=true];  // 设备id,留空表示全部,可多选
...
message rqQueryEventsTimetableDo {
    ...
    int32 data_type = 4;  // 数据标记，0-全部，1-设备应答，3-设备成功应答，2-设备无应答写入最后时限时间
    int64 dt_start = 5;  // 查询起始日期
    int64 dt_end = 6;  // 查询结束日期
}
```

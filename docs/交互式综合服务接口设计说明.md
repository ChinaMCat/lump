...
- 返回:
    ...
    message UserLogin {
        ...
        int32 is_user_operator_code = 19;  
        string user_operator_code = 20;
    }
    ```
...
- 参数:
    ...
    message rqSluCtl {
        ...
        repeated int32 week_set = 8 [packed=true];  // 周设置,周日～周六，要操作的日期置1,不操作置0
        int32 timer_or_offset = 9;  // 定时 hh*60+mm->int32 或偏移量 依据 operation_type定
        int32 cmd_type = 10;  // 操作类型 3-经纬度关灯，4-混合控制，5-pwm调节，6-485调节
        repeated int32 cmd_mix = 11 [packed=true];  // 混合回路操作1-4回路 0-不操作，1-开灯，2-1档节能，3-2档节能，4-关灯（经纬度关灯时，cmd_type<4视为不操作）
...
- 参数:
    ...
    message rqLduDataGet {
        Head head = 1;
        repeated int32 tml_id = 2 [packed=true];  // 要操作的设备逻辑地址列表,该列表中的终端均执行相同的操作
        repeated int32 loop_id = 3 [packed=true];  // 回路标示，必填，3090设备范围1-6，1090设备范围1-2,示例：[1,2,3,6]
    }
    ```
...
- 参数:
    ...
    message rqEluCtl {
        Head head = 1;
        repeated int32 tml_id = 2 [packed=true];  // 要操作的设备逻辑地址列表,该列表中的终端均执行相同的操作
        repeated int32 loop_id = 3 [packed=true];  // 填写1-8回路操作标示,0-关,1-开,2-不操作
    }
    ```
...
- 返回:
    ...
    message QueryDataErr {
        ...
        Head head = 1;
        int32 type = 2;  // 类型,0-现存,1-历史,2-现存仅返回数量（存放在head.paging_record_total），3-历史仅返回数量（存放在head.paging_record_total）
...
- 返回:
    ...
    message QueryEventsTimetableDo {
        message TimetableDoView {
            ...
            int64 dt_reply = 6;  // 设备应答时间，若未应答，则返回0或系统设置时间（废弃）
        }
        Head head = 1;
...
- 权限需求：
	1.	有效权限范围为（2，3，6，7，15）

- 参数:
	1.	uuid - 登录成功获得的动态id
	2.	pb2 - 详细参数(结构如下)
    ```protobuf
    message rqSluitemAdd {
        Head head = 1;
        int64 slu_id = 2;  // 集中器id
        repeated SluItemArgs sluitem_args = 3;  // 控制器参数
        message SluItemArgs {
            int64 sluitem_barcode = 2;  // 条码
            int32 sluitem_power_uplimit = 3;  // 功率上限
            int32 sluitem_power_lowlimit = 4;  // 功率下限

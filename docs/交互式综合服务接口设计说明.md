...
-	zeroMQ 过滤器格式定义(暂定)
	...
	1.	整体格式采用`服务名.req/rep.<详细标记>`的格式。服务名列表见下。
	2.	`req`表示向末端设备请求，`rep`表示末端数据的应答，二选一。
	3.	各个服务的详细标记定义见各自协议部分。
...
-	推送给客户端的终端应答消息体： 采用MsgWithCtl()结构发送
...
> zmq-pub 广播过滤器格式: `jkdb.rep.alarm.故障产生时间.1-报警/0-消警.故障id.设备id`  
> zmq-pub 广播消息体格式: `rqSubmitAlarm.AlarmView() 结构体序列化后的二进制字符串`, 即一条推送仅包含一条故障信息
...
> 接口名称: mrudataget  
> 发送电表抄表指令, 结果可通过两种方式获得,1. 直接解析zmq-pub推送信息,可获得实时结果;2. 提交请求后,等待系统将结果写入数据库后,调用查询接口获得. 注: 因电表采用485中转通信,因此无论何种方式可能均需要4-5s才能获得数据  
> zmq-pub 广播过滤器格式: `tcs.rep.wlst.mru.9100.电表地址`  
> zmq-pub 广播消息体格式: `MsgWithCtrl() 结构体序列化后的二进制字符串`, 一条推送仅包含一条抄表信息  
> MsgWithCtrl()结构体见附件
...
zmq-pub 过滤器: `sz.msg_type.msg_receiver` 若有中文,以utf8进行编码
...
> zmq-pub 广播过滤器格式: `tcs.rep.端口号.cmd`,cmd参考新版监控通信层与数据层通信协议  
> zmq-pub 广播消息体格式: `MsgWithCtrl() 结构体序列化后的二进制字符串`, 一条推送仅包含一条应答

---

#### 设备信息更新推送

> zmq-pub 广播过滤器格式: `jkdb.rep.tmlinfo.设备id.更新内容（参考TmlInfo.data_mark，多项内容分多条推送）`  
> zmq-pub 广播消息体格式: `TmlInfo() 结构体序列化后的二进制字符串`, 一条推送仅包含一条设备信息
...
> zmq-pub 广播过滤器格式: `jkdb.rep.alarm.故障产生时间.1-报警/0-消警.故障id.设备id`  
> zmq-pub 广播消息体格式: `rqSubmitAlarm.AlarmView() 结构体序列化后的二进制字符串`, 一条推送仅包含一条故障信息

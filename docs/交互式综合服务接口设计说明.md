...
- 返回:
    ...
    message QueryDataRtu {
        message LoopView {
			...
			int32 loop_switchout_id = 11;  // 回路对应开关量输出（1-16），0-表示无开关量输出
        }
        message DataRtuView {
            int32 tml_id = 1;
            int64 phy_id = 2;
...
- 返回:
    ...
    message QueryDataErr {
        message ErrView {
			...
			double voltage = 14; // 电压/火线电流
			double current = 15;  // 电流/零线电流
			string err_name_custome = 16;  // 自定义故障名称
			string tml_loop_name = 17;  // 回路名称，如果有的话
        }
        Head head = 1;
...
- 参数:
    ...
    message rqQueryDataEvents {
        ...
        int64 dt_start = 5;  // 起始年月日时间,转换为long传输(1970-1-1开始的秒数)
        int64 dt_end = 6;  // 结束年月日时间,转换为long传输(1970-1-1开始秒数)
        repeated int32 events_id = 7 [packed=true];  // 不填充时查询全部
        repeated string user_name = 8;  // 用户名,可多选。不填充时查询全部
        repeated int32 tml_id = 9 [packed=true];  // 设备id,可多选，不填充时查询全部
    }
    ```

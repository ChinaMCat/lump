                            ...
                            if record_total is None:
                                ...
                                for d in cur:
                                    ...
                                    if mk == 6:
                                        ...
                                        iteminfo.sluitem_st_poweron.extend([d[30], d[31], d[32], d[
                                            33]])
                                        iteminfo.sluitem_st = d[34]
                                        iteminfo.sluitem_alarm = d[35]
                                        iteminfo.sluitem_vector.extend([d[36], d[37], d[38], d[39]])
                                        iteminfo.sluitem_rated_power.extend([d[41], d[42], d[43], d[
                                            44]])
...
where EXISTS (select rtu_id,date_create from (SELECT rtu_id,max(date_create) as date_create 
FROM {0}_data.data_rtu_view group by rtu_id) as t 
...
order by a.rtu_id,a.date_create desc,a.loop_id'''.format(
                ...
                if record_total is None:
                    ...
                    msg.head.paging_total = paging_total
                    for d in cur:
                        if drv.tml_id != d[1] or drv.dt_receive != mx.switchStamp(d[0]):
                            if drv.tml_id > 0:
                                msg.data_rtu_view.extend([drv])
                                drv = msgws.QueryDataRtu.DataRtuView()
                            drv.tml_id = d[1]

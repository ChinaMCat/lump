                ...
                if msg.head.if_st == 1:
                    ...
                    for mk in rqmsg.data_mark:
                        if mk in (1, 3):  # 基础信息/仅tml_id和tml_dt_update
                            ...
                            on a.rtu_id=b.rtu_id {1}'.format(utils.m_dbname_jk, str_tmls)
                            ...
                            if record_total is None:
                                ...
                                for d in cur:
                                    ...
                                    # 加入/更新地址对照缓存
                                    self.set_phy_list(int(d[0]), (int(d[1]), int(d[7]), d[3]))
                            ...
                            from {0}.para_base_equipment {1}'.format(utils.m_dbname_jk, str_tmls)
                            record_total, buffer_tag, paging_idx, paging_total, cur = yield self.mydata_collector(
                                strsql,
                                need_fetch=1,
                                need_paging=0)
                            ...
                            order by a.rtu_id'.format(utils.m_dbname_jk, str_tmls)
                            ...
                            order by slu_id,grp_id'.format(utils.m_dbname_jk, str_tmls)
        ...
        if user_data is not None:
            if user_data['user_auth'] in utils._can_read:
                ...
                if rqmsg.data_mark == 0:
                    ...
                    order by a.date_create desc,a.rtu_id,a.loop_id'''.format(utils.m_dbname_jk, sdt,
                                                                             edt, str_tmls)
                else:
                    ...
                    order by a.rtu_id,a.loop_id'''.format(utils.m_dbname_jk, sdt, edt, str_tmls)
        ...
        if user_data is not None:
            if user_data['user_auth'] in utils._can_read:
                ...
                if rqmsg.type == 0:  # 最新数据
                    ...
                    #         msg.head.if_st = 46
                    #     else:
                    #         str_tmls = ' rtu_id={0}'.format(rqmsg.tml_id[0])
                    # else:
                    ...
                    #         msg.head.if_st = 46
                    #     else:
                    #         str_tmls = ' a.slu_id={0}'.format(tml_ids.pop())
                    # 
                    # strsql = 'select a.date_create,a.rtu_id, \
                    #             a.rtu_voltage_a,a.rtu_voltage_b,a.rtu_voltage_c, \
                    ...
                    #             c.loop_name from {0}_data.data_rtu_view as a left join {0}.para_rtu_loop_info as c on a.rtu_id=c.rtu_id and a.loop_id=c.loop_id \
                    #             where a.date_create=(select max(date_create) from {0}_data.data_rtu_record where {1})'.format(
                    #     utils.m_dbname_jk, str_tmls)
                    ...
                    #         if drv.tml_id != d[0]:
                    #             drv.tml_id = d[1]
                    #             drv.dt_receive = mx.switchStamp(d[0])
                    #             drv.voltage_a = d[2]
                    #             drv.voltage_b = d[3]
                    #             drv.voltage_c = d[4]
                    #             drv.current_sum_a = d[5]
                    #             drv.current_sum_b = d[6]
                    #             drv.current_sum_c = d[7]
                    #             drv.alarm_st.extend([int(a) for a in '{0:08b}'.format(d[8])[::-1]])
                    ...
                    #             drlv.factor = d[14]
                    #             drlv.rate = d[15]
                    #             drlv.switch_in_st = d[16]
                    #             drlv.current_over_range = d[17]
                    #             drlv.voltage_over_range = d[18]
                    #             drlv.loop_name = d[19]
                    ...
                    where a.date_create>={1} and a.date_create<={2} {3} 
                    order by a.rtu_id,a.date_create desc,a.loop_id'''.format(utils.m_dbname_jk, sdt,
                                                                             edt, str_tmls)
                ...
                if record_total is None:
                    ...
                    msg.head.paging_total = paging_total
                    for d in cur:
                        if drv.tml_id != int(d[1]) or drv.dt_receive != mx.switchStamp(int(d[0])):
                            if drv.tml_id > 0:
                                msg.data_rtu_view.extend([drv])
                                drv = msgws.QueryDataRtu.DataRtuView()
                            drv.tml_id = int(d[1])
                            ...
                            drv.alarm_st.extend([int(a) for a in '{0:08b}'.format(int(d[8]))[::-1]])
                            x = d[9][:len(d[9]) - 1].split(';')
                            drv.switch_out_st.extend([1 if a == 'True' else 0 for a in x])
                        # if d[19] is not None:
                        drlv = msgws.QueryDataRtu.LoopView()
                        drlv.loop_id = int(d[10])
                        drlv.voltage = float(d[11])
                        drlv.current = float(d[12])
                        drlv.power = float(d[13])
                        ...
                        drlv.loop_name = d[19] if d[19] is not None else ''
                        drv.loop_view.extend([drlv])
                        del drlv
                    if drv.tml_id > 0:
                        msg.data_rtu_view.extend([drv])

...
@mxweb.route()
class QueryDataRtuGZHandler(base.RequestHandler):

    help_doc = u'''终端运行数据查询-广州专用 (post方式访问)<br/>
    <b>参数:</b><br/>
                    ...
                    if record_total is None:
                        ...
                        for d in cur:
                            if drv.tml_id != int(
                                ...
                                drv.phy_id = int(
                                    d[24]) if d[24] is not None else -1
                                drv.tml_name = d[
                                    25] if d[25] is not None else ''
                                drv.temperature = d[19]
                                drv.total_power = d[20]
                                drv.control_type = d[21]
                                drv.electricity = d[22]
                            # if d[19] is not None:
                            if d[10] is not None:
                                ...
                                drlv.loop_name = d[
                                    23] if d[23] is not None else ''
                                drlv.loop_switchout_id = d[
                                    26] if d[26] is not None else 0
                    ...
                    if record_total is None:
                        ...
                        for d in cur:
                            ...
                            if d[10] is not None:
                                ...
                                drlv.voltage_over_range = int(d[18])
                                drlv.loop_name = d[
                                    20] if d[20] is not None else ''
                                drlv.loop_switchout_id = d[
                                    23] if d[23] is not None else 0
                                drv.loop_view.extend([drlv])
                                del drlv
                        if drv.tml_id > 0:
                            msg.data_rtu_view.extend([drv])
...
class RtuTimerCtlHandler(base.RequestHandler):
    ...
    def post(self):
        ...
        if env and rqmsg.data_mark == 1:
            self.write_event(
                11, contents, 2, user_name=user_data['user_name'], app_unique=rqmsg.head.unique, device_ids=device_ids, remark=u"交互接口命令")
        del msg, rqmsg, user_data
